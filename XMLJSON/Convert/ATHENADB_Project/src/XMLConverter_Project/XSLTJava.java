...
public class XSLTJava {
	public void convertXSLT(String filename, String path) {
		  try {
	            ...
	            File inputFile = new File(s);// a file with .xml is created with each doi as name in the current directory
	            File tempFile = new File(path+"/"+"tempFile.xml");// a tempfile is also created

	             BufferedReader reader = new BufferedReader(new FileReader(inputFile)); // FileReader is meant for reading streams of characters in a file. 
	            ...
	            while((currentLine = reader.readLine()) != null)// readline return Null or a string
	             {
	                 ...
	                 {
	                	 // replace doctype line by a comment line becaue we don't handle the DTD
