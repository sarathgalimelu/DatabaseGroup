...
public class List_File {
	...
	{
		String filename="";// would content the file name with no extension
		...
		// read contents of file array if file show it
		for (File file:listofFile)// for each file in array of file called listofFile
		{
			if (file.isFile() && file.getName().endsWith(".xml"))// if it is a file and it end with .xml
			{
                ...
                //------------------------------------------------------------// 
  	             // remove extension .xml on file name before using to convert because the ConvertXslT function take file name without extension
				    ...
				    //filename = file.getName().substring(0, file.getName().lastIndexOf("."));// the name of the file would be from the 1st character to the character before .
